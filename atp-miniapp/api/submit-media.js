...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (!phone || !fileBase64) return res.status(400).json({ error: 'missing phone or file' });
    if (!BOT_TOKEN || !SALES_CHAT_ID) {
      console.error('Thiếu BOT_TOKEN hoặc SALES_CHAT_ID');
    ...
    if (user && user.id) {
      await sendTelegramMessage(user.id,
        `Cảm ơn anh/chị! ATP Steel đã nhận ảnh/tài liệu hỏi giá (<b>${escapeHtml(ticket)}</b>). Nhân viên kinh doanh sẽ liên hệ qua số ${escapeHtml(phone)} sớm nhất.`);
    }
